    ...
    def rotate(self, matrix: List[List[int]]) -> None:
        ...
        while n - 2*offset > 1:
            for i in range(n-1-2*offset):
                matrix[offset][offset+i], matrix[n-1-offset-i][offset] = matrix[n-1-offset-i][offset], matrix[offset][offset+i]
                matrix[n-1-offset-i][offset], matrix[n-1-offset][n-1-offset-i] = matrix[n-1-offset][n-1-offset-i], matrix[n-1-offset-i][offset]
                matrix[n-1-offset][n-1-offset-i], matrix[offset+i][n-1-offset] = matrix[offset+i][n-1-offset], matrix[n-1-offset][n-1-offset-i]
